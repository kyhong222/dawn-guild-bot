...
class Subway(commands.Cog):
    ...
    def _status(self, now: datetime) -> tuple[str, str]:
        ...
        # 다음 탑승: 다음 offset 5 시각, 그 사이클의 출발(+5분) + 운행
        next_board = self._next_time(now, 5, cycle)
        # 다음 탑승의 도착 = 다음 탑승 시각 기준으로 출발(+5분) + 운행
        # offset 5에서 출발까지 5분 + travel
        next_board_min = int(next_board.split(":")[1])
        next_board_hour = int(next_board.split(":")[0])
        arrive_total = next_board_min + 5 + travel
        arrive_h = (next_board_hour + arrive_total // 60) % 24
        arrive_m = arrive_total % 60
        next_arrival = f"{arrive_h:02d}:{arrive_m:02d}"
